...
const PrayerTimesPage: React.FC = () => {
    ...
    useEffect(() => {
        adzanAudioRef.current = new Audio('/regular.mp3');
        return () => {
        ...
        if (schedule) {
            ...
            if (foundActive && foundActive !== notifiedPrayer) {
                ...
                if (isAdzanEnabled) {
                    ...
                    if (adzanAudioRef.current) {
                        // Differentiate Subuh Adzan (Fajr has different lyrics)
                        const audioUrl = foundActive === 'Subuh'
                            ? '/subuh.mp3' // Subuh / Fajr
                            : '/regular.mp3'; // Regular

                        adzanAudioRef.current.src = audioUrl;
