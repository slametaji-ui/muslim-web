...
const MobileNavLink = ({ to, icon, label, active }: { to: string; icon: React.ReactNode; label: string; active: boolean }) => (
    <Link
        to={to}
        className={`flex flex-col items-center gap-1 py-1 px-2 rounded-2xl transition-all duration-500 relative flex-1 ${active ? 'text-emerald-600 dark:text-emerald-400 scale-105' : 'text-slate-400 dark:text-slate-500 hover:text-emerald-500'
            }`}
    >
        {active && (
        ...
        <span className={`text-[7px] font-black uppercase tracking-[0.1em] transition-all duration-500 ${active ? 'opacity-100 translate-y-0.5' : 'opacity-40'}`}>
            {label}
        </span>
        {active && (
